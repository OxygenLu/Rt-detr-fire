...
try:
    font = ImageFont.truetype("Tests/fonts/FreeMono.ttf", 30)
except IOError:
...
class ImageReader:
    def __init__(self, resize=224, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        ...
        self.resize = resize
        self.pil_img = None   #store the last read image
...
def get_argparser():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="rt-DETR/rtdetr_pytorch/configs/rtdetr/rtdetr_r50vd_6x_coco.yml", help="config")
    parser.add_argument("--ckpt", default="rt-DETR/rtdetr_pytorch/weights/rtdetr_r50vd_6x_coco_from_paddle.pth", help="ckpt_path")
    parser.add_argument("--image", default="rt-DETR/rtdetr_pytorch/images/keji.jpg", help="test_image_path")
    parser.add_argument("--output_dir", default="./", help="save_path")
...
def main(args):
    ...
    print(f"Inf time:{time.time() - start:.4f}s")
    labels, boxes, scores = output
    im = reader.pil_img
    draw = ImageDraw.Draw(im)

    thrh = 0.5# define a threshold
    ...
    for i in range(img.shape[0]):
        ...
        for b in box:
            ...
            draw.text((b[0], b[1]), text="fire", fill='blue', font=font,embedded_color=True)
